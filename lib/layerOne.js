...
export function buildActions({ appearanceSummary, yourDestinations, trustedSources, sentiment }) {
  const appearedIn = appearanceSummary?.appearedIn ?? 0;
  const totalAttempted = appearanceSummary?.totalAttempted ?? 0;
  ...
  if (appearedIn === 0) {
    candidates.push(
      topSource ? `Get your product reviewed on ${topSource}.` : "Get listed on real review sites — AI trusts them."
    );
  }
  if (appearedIn > 0 && brandDirectCount === 0) {
    candidates.push("Your website never appears — add your products clearly.");
  }
  if (sentiment && sentiment.sentiment === "negative") {
    candidates.push(`AI calls you "${sentiment.positioning}" — that needs fixing.`);
  }
  if (appearedIn > 0 && appearedIn < totalAttempted) {
    candidates.push("You show up sometimes — be consistent everywhere.");
  }
  if (candidates.length < 2 && topSource) {
    candidates.push(`Get featured on ${topSource} too.`);
  }
  if (candidates.length < 2 && appearedIn > 0) {
    candidates.push("Keep your product details fresh so AI keeps recommending you.");
  }
  if (candidates.length < 2) {
    candidates.push("Test again monthly — AI answers change often.");
  }
  const actions = candidates.slice(0, 2);
  actions.push("Ask us how to fix this — reply to your report email.");
  return actions;
}

// One call for all four cards at once — used by both StoryView.js and
// app/api/lead/route.js's email builder.
export function buildLayerOne({ brand, report, engines, sentiment, trustedSources }) {
