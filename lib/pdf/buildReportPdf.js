...
export async function buildReportPdf({
  ...
  if (checkoutTotal > 0) {
    ...
    const destColors = { "brand-direct": GOOD, marketplace: BAD, aggregator: "#4a9fd8", none: "#3a5a48" };
    y = drawStackedBar(
      doc,
      y,
  ...
  if (sov && sov.slotTotal > 0) {
    ...
    const segments = [
      ...
      { pct: sov.rival, color: BAD },
      { pct: sov.other, color: "#3a5a48" },
    ];
    ...
    y = drawLegendRow(doc, y, rivalLabel, `${sov.rival}%`, BAD);
    y = drawLegendRow(doc, y, "Everyone else", `${sov.other}%`, "#3a5a48");
    y += 10;
  }
  ...
  if (sentiment && (mentionCount ?? 2) >= 2) {
    ...
    if (sentiment.summary) y = drawParagraph(doc, sentiment.summary, y, { size: 9.5, color: MUTED, gap: 20 });
  }
  ...
  if (hasAnyQuestions) {
    ...
    ENGINE_ORDER.forEach((engine) => {
      ...
      rows.forEach((row) => {
        ...
        y = doc.y + 2;
        doc.font("Helvetica").fontSize(8.5).fillColor(MUTED).text(rText, PAGE_MARGIN, y, {
          width: contentWidth(doc),
        });
        y = doc.y + 10;
      });
