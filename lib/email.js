...
// mentioned the brand at all (appearance.verdict — Claude's live
...
function subjectFor(brand, appearance, founder) {
  const verdict = appearance?.verdict;
...
// data-free version.
export function buildMerchantEmail({ brand, category, market, appearance, founder, reportUrl, brandWebsite }) {
  const marketLabel = MARKET_LABELS[market] || market;
  const subject = subjectFor(brand, appearance, founder);
...
export async function sendLeadEmails({
  ...
  const results = await Promise.allSettled([
    ...
    (() => {
      const merchantEmail = buildMerchantEmail({ brand, category, market, appearance, founder, reportUrl, brandWebsite });
...
// Phase 2 — merchant magic-link login (lib/auth/session.js). Deliberately
// bare: one link, one sentence, a 15-minute expiry note — this is a login
// mechanism, not a marketing moment.
export async function sendMagicLinkEmail({ email, link }) {
  const client = resend();
  if (!client) {
    console.log("[email] RESEND_API_KEY not set — cannot send magic link.");
    return { sent: false };
  }
  const fromEmail = process.env.FROM_EMAIL || "StockedBy <onboarding@resend.dev>";
  try {
    const res = await client.emails.send({
      from: fromEmail,
      to: email,
      subject: "Your StockedBy sign-in link",
      text: `Sign in to StockedBy:\n\n${link}\n\nThis link expires in 15 minutes and can only be used once. If you didn't request this, you can ignore this email.`,
      html: `<p>Sign in to StockedBy:</p><p><a href="${esc(link)}">${esc(link)}</a></p><p>This link expires in 15 minutes and can only be used once. If you didn't request this, you can ignore this email.</p>`,
    });
    return { sent: !res?.error };
  } catch (e) {
    console.error("[email] magic link send failed", e?.message || e);
    return { sent: false };
  }
}
