...
export function buildMerchantEmail({ brand, category, market, layer1, reportUrl, brandWebsite }) {
  ...
  if (!layer1) {
    return {
      ...
      html: `<p>Hi,</p><p>Your StockedBy test for <b>${esc(brand)}</b> in ${esc(categoryMidSentence(category))} (${esc(marketLabel)}) is done.</p>${reportLineHtml}${nextProductLineHtml}${signOffHtml}`,
    };
  }

  const { appearance, brands, destinations, emailTips } = layer1;
  const opening = appearanceLine(appearance);
  // topOthers excludes the brand's own name (lib/layerOne.js's
  // buildTopBrands) — "AI recommended instead" only makes sense as a list
  // of who ELSE got recommended, not a list that might just repeat the
  // merchant's own brand back at them.
  const brandNames = brands.topOthers.map((b) => b.label);
  const brandsLine = brandNames.length > 0 ? `AI recommended instead: ${brandNames.join(", ")}.` : "";
  ...
  const textParts = [
    "Hi,",
    "",
    `We asked ChatGPT, Gemini and Claude what shoppers should buy in ${categoryMidSentence(category)} in ${marketLabel}.`,
    opening,
    "",
  ];
  ...
  const htmlParts = [
    "<p>Hi,</p>",
    `<p>We asked ChatGPT, Gemini and Claude what shoppers should buy in ${esc(categoryMidSentence(category))} in ${esc(marketLabel)}.<br>${esc(opening)}</p>`,
  ];
...
export function buildDeveloperFixEmail({ domain, platform, products, llmsTxt, merchantEmail }) {
  ...
  doneProducts.forEach((p) => {
    ...
    htmlParts.push(
      ...
      `<pre style="background:#0e1f18;color:#f1f5ec;padding:12px;border-radius:8px;overflow-x:auto;font-size:12px">&lt;script type="application/ld+json"&gt;\n${esc(code)}\n&lt;/script&gt;</pre>`
    );
  });

  if (llmsTxt) {
    textParts.push(`--- llms.txt ---`, llmsTxt, ``);
    htmlParts.push(`<h3>llms.txt</h3>`, `<pre style="background:#0e1f18;color:#f1f5ec;padding:12px;border-radius:8px;overflow-x:auto;font-size:12px">${esc(llmsTxt)}</pre>`);
