...
export function buildMerchantEmail({ brand, category, market, layer1, reportUrl }) {
  ...
  if (!layer1) {
    return {
      subject,
      text: `Hi,\n\nYour StockedBy test for ${brand} in ${category} (${marketLabel}) is done.\n\n${reportLineText}${signOff}`,
      html: `<p>Hi,</p><p>Your StockedBy test for <b>${esc(brand)}</b> in ${esc(category)} (${esc(marketLabel)}) is done.</p>${reportLineHtml}${signOffHtml}`,
    };
  }
...
export async function sendLeadEmails({ email, brand, market, category, painpoint, verdict, reportSlug, layer1 }) {
  ...
  const results = await Promise.allSettled([
    ...
    (() => {
      const merchantEmail = buildMerchantEmail({ brand, category, market, layer1, reportUrl });
