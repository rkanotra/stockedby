...
import { MARKET_LOCALE } from "./scoring.js";
...
// 1.5 hardening) — the model actually used for the live shopping-assistant
// call, never re-derived or guessed elsewhere.
...
// Errors thrown here and by sanityCheckRecs() below carry a `.kind` tag
// ("parse_failure" | "sanity_rejection") so callers (app/api/test/query,
// app/api/generate-queries) can log a real system_events row per failure
// type — self-improvement infra, supabase/migrations/0003.
function extractJSON(content) {
  ...
  if (start === -1 || end === -1) {
    const err = new Error("No JSON in Claude reply");
    err.kind = "parse_failure";
    throw err;
  }
  try {
...
// Never fabricate (rule 2): an empty or all-blank "recommendations" array
// is a real failure to surface, not a valid 0-result answer.
function sanityCheckRecs(recs) {
  if (!Array.isArray(recs) || recs.length === 0) return false;
  return recs.some((r) => r?.brand || r?.product);
}
...
{"recommendations":[{"brand":"","product":"","why":"","destination":"","destination_domain":""}]}`;
  ...
  if (!sanityCheckRecs(recs)) {
    const err = new Error("Empty or garbage recommendations from Claude");
    err.kind = "sanity_rejection";
    throw err;
  }
  return { recs, searches, citations };
}
...
export async function generateCustomQueries({ market, categoryName, brand }) {
  ...
  if (queries.length !== 4) {
    const err = new Error("Query generation returned an unexpected number of questions — please try again.");
    err.kind = "sanity_rejection";
    throw err;
  }
  ...
  if (JSON.stringify(gotArchetypes) !== JSON.stringify([...CUSTOM_QUERY_ARCHETYPES].sort())) {
    const err = new Error("Query generation returned unexpected question types — please try again.");
    err.kind = "sanity_rejection";
    throw err;
  }
  ...
  return queries.map((q) => {
    ...
    if (!text || text.includes("{") || text.includes("}")) {
      const err = new Error("Query generation returned an unfilled or empty question — please try again.");
      err.kind = "sanity_rejection";
      throw err;
    }
