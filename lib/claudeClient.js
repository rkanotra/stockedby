...
// timeoutMs is caller-supplied (app/api/test/route.js uses a shorter one on
// its single retry attempt) so the retry's own budget can be tuned
// independently of the first attempt's.
export async function askShoppingAssistant(queryText, { timeoutMs = 40_000 } = {}) {
...
{"recommendations":[{"brand":"","product":"","why":"","destination":"","destination_domain":""}]}`;
  ...
  // The route's maxDuration budgets for one retry per query, plus room for
  // a pause_turn continuation on top (see its own comment) — if Claude's
  // web search runs long, Vercel kills the whole function and every
  // already-finished query is lost too. Cap each call well under that
  // budget so a slow query fails on its own (handled per-query by the
  // caller) instead of taking the whole run down.
  ...
  // well past the route's ceiling — before ever surfacing as an error. The
  // caller (route.js) does its own single retry instead, with a shorter
  // timeout and knowledge of *why* the first attempt failed (e.g. a 429).
  ...
  // gives the model room to finish. Capped at 1 continuation with its own
  // short timeout (finishing a paused turn is fast — no more searching,
  // just wrapping up text) so this can't itself blow the route's budget.
  const allContent = [...res.content];
  if (res.stop_reason === "pause_turn") {
    messages.push({ role: "assistant", content: res.content });
    res = await client().messages.create(
      { model: MODEL, max_tokens: 2000, messages, tools },
      { timeout: Math.min(timeoutMs, 10_000), maxRetries: 0 }
    );
