...
{"recommendations":[{"brand":"","product":"","why":"","destination":"","destination_domain":""}]}`;
  ...
  // The route's maxDuration budgets for one retry per query (see its own
  // comment) — if Claude's web search runs long, Vercel kills the whole
  // function and every already-finished query is lost too. Cap each call
  // well under that budget so a slow query fails on its own (handled
  // per-query by the caller) instead of taking the whole run down.
