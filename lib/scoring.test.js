...
// Phase 1 "Foundation" — computeReport's new destinationsByEngine must sum
// back to the same combined destTally/destinations the report already
// returned (no double-counting, no dropped rows across the per-engine
// refactor), while still exposing a real per-engine breakdown for cases
// where engines genuinely disagree on routing.
test("computeReport: destinationsByEngine sums back to the combined destinations tally", () => {
  const brand = "TestBrand";
  const claudeRows = [
    { qid: "q1", archetype: "branded-routing", recs: [{ brand, product: "", why: "", destination: "brand-direct", destination_domain: "brand.com" }] },
  ];
  const chatgptRows = [
    { qid: "q1", archetype: "branded-routing", recs: [{ brand, product: "", why: "", destination: "marketplace", destination_domain: "amazon.in" }] },
  ];
  const engineData = { claude: claudeRows, chatgpt: chatgptRows, gemini: [] };
  const appearanceSummary = computeAppearanceSummary(
    [{ qid: "q1", archetype: "branded-routing", status: "done", recs: claudeRows[0].recs }],
    brand
  );
  const report = computeReport({
    market: "India",
    ...
    engineData,
    organicEngineData: { claude: [], chatgpt: [], gemini: [] },
    appearanceSummary,
  });

  assert.equal(report.destinationsByEngine.claude.tally["brand-direct"], 1);
  assert.equal(report.destinationsByEngine.chatgpt.tally.marketplace, 1);
  assert.equal(report.destinationsByEngine.gemini.total, 0);

  // Combined tally must equal the sum of every engine's own tally.
  ["brand-direct", "marketplace", "aggregator", "none"].forEach((key) => {
    const summed = Object.values(report.destinationsByEngine).reduce((sum, e) => sum + e.tally[key], 0);
    assert.equal(report.destinations.tally[key], summed);
  });
});
