...
// A human-readable marketplace name for a KNOWN marketplace domain
// ("nykaa.com" -> "Nykaa", "amazon.ae" -> "Amazon") — a deterministic
// string transform of data lib/marketProfiles.js already asserts is a real
// marketplace, never a guess at an unrecognized domain (returns null then).
// lib/observations.js uses this so a persisted observation can say WHICH
// marketplace a buyer was routed to, not just the generic "marketplace"
// bucket (Phase 1.5 hardening).
export function marketplaceLabelForDomain(domain) {
  const d = normalizeDomain(domain);
  if (!d) return null;
  if (isAmazonDomain(d)) return "Amazon";
  const root = KNOWN_MARKETPLACE_ROOTS.find((r) => matchesRoot(d, r));
  if (!root) return null;
...
export function computeReport({
  ...
  // Looped per-engine (rather than one flatMap across ENGINE_ORDER) so the
  // same pass that builds the combined destTally/yourDest below also
  // builds destinationsByEngine — a per-engine breakdown of the exact same
  // own-site-vs-marketplace routing classification, added for the
  // AI-observation-model improvement (market-expansion foundation phase).
  // Purely additive: destTally/destTotal/yourDest are summed across engines
  // in the same order as the old single flatMap, so every existing reader
  // of `destinations` sees byte-identical values.
  const destinationsByEngine = {};
  const destTally = { "brand-direct": 0, marketplace: 0, aggregator: 0, none: 0 };
  const yourDest = {};
  let destTotal = 0;

  ENGINE_ORDER.forEach((engine) => {
    const rows = engineData[engine] || [];
    const engineTally = { "brand-direct": 0, marketplace: 0, aggregator: 0, none: 0 };
    rows.forEach((r) =>
      r.recs.forEach((rec) => {
        destTotal++;
        const isYou = matches(brand, rec?.brand) || matches(brand, rec?.product);
    ...
    const engineTotal = Object.values(engineTally).reduce((a, b) => a + b, 0);
    destinationsByEngine[engine] = {
      tally: engineTally,
      total: engineTotal,
      pct: Object.fromEntries(
        Object.entries(engineTally).map(([k, v]) => [k, engineTotal ? Math.round((v / engineTotal) * 100) : 0])
      ),
    };
  });
