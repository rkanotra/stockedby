...
export function computeReport({
  ...
  const organicRows = ENGINES.flatMap((e) => organicEngineData[e] || []);
  ...
  const destinationRows = ENGINES.flatMap((e) => engineData[e] || []);
...
export function buildFounderSummary({ brand, category, appearanceSummary, yourDestinations }) {
  ...
  if (appeared === 0) {
    const cat = category ? ` when shoppers ask about ${category}` : "";
