...
test("buildDestinationSplitByEngine: only includes engines with real destination data, never a forced split", () => {
  const byEngine = {
    claude: { tally: { "brand-direct": 3, marketplace: 1, aggregator: 0, none: 0 } },
    chatgpt: { tally: { "brand-direct": 0, marketplace: 0, aggregator: 0, none: 5 } },
    gemini: { tally: { "brand-direct": 0, marketplace: 0, aggregator: 0, none: 0 } },
  };
  const out = buildDestinationSplitByEngine(byEngine);
  assert.ok(out.claude);
  assert.equal(out.claude.ownSitePct, 75);
  assert.equal(out.chatgpt, undefined);
  assert.equal(out.gemini, undefined);
});

test("buildDestinationSplitByEngine: no engine data at all -> null", () => {
  assert.equal(buildDestinationSplitByEngine(null), null);
  assert.equal(
    buildDestinationSplitByEngine({ claude: { tally: { "brand-direct": 0, marketplace: 0, aggregator: 0, none: 0 } } }),
    null
  );
});
