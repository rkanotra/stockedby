...
// /api/test's maxDuration is 100s (padded for the live Claude calls' own
// retry budget — see route.js) — same reasoning as lib/claudeClient.js's
// Claude timeout: keep each call well under that and disable SDK retries,
// so a slow/stuck call fails on its own instead of eating the whole
// request's budget (a default-retried timeout can silently run 2-3x this).
// Harvest jobs don't get the query-retry treatment (see route.js) — their
// own "runs at most once" cost guard is intentional and stays as-is.
