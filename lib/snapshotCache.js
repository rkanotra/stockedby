...
export async function fetchCachedSnapshots(market, categoryId) {
  ...
  try {
    const { data, error } = await db
      .from("snapshots")
      .select("id, qid, engine, collected_on, snapshot_json")
      .eq("market", market)
    ...
    return data.map((row) => ({
      ...
      // Real Supabase row id, threaded through to app/api/test/route.js's
      // engineData and on to lib/observations.js as `snapshotId` — a real
      // provenance pointer (Phase 1.5 hardening), not present when this
      // snapshot came from a bank file's inline seed instead of this table.
      snapshotDbId: row.id,
    }));
  } catch (e) {
...
// with equivalent data instead of erroring. Returns the row's real
// Supabase id (or null on failure) so app/api/test/route.js can pass a
// genuine `snapshotId` through to lib/observations.js — never fabricated.
export async function writeThroughSnapshot({ market, categoryId, qid, engine, collectedOn, recommendations, sources }) {
  const db = supabase();
  if (!db || !categoryId) return { id: null };
  try {
    const { data, error } = await db
      .from("snapshots")
      .upsert(
      ...
      .single();
    if (error) {
      console.error("[snapshot-cache] write-through failed", error.message);
      return { id: null };
    }
